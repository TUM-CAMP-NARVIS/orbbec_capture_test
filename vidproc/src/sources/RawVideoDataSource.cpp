#include "tcn/vpf/sources/RawVideoDataSource.h"
...
namespace tcn::vpf
{
	...
	void RawVideoDataSource::Init(int width, int height, AVPixelFormat _sourcePixelFormat, AVPixelFormat targetPixelFormat, int framesPerSecond, FrameSink* _output)
	{
		output = _output->CreateStream();
	...
	void RawVideoDataSource::WriteFrame(void* data, int bytesPerRow)
	{
		...
		// if the src and target pixel format are the same, we don't do any conversions, we just copy
