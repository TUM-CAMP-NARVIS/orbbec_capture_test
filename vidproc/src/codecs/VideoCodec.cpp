...
namespace tcn::vpf
{
	...
	OpenCodec* VideoCodec::Open(int width, int height, const Rational& frameRate, AVPixelFormat format)
	{
		...
		// sanity checks
		if (!IsPixelFormatSupported(format)) throw FFmpegException("Pixel format " + std::string(av_get_pix_fmt_name(format)) + " is not supported by codec " + codecContext->codec->name);
		if (!IsFrameRateSupported(frameRate)) throw FFmpegException("Frame rate " + std::to_string(frameRate.numerator()) + "/" + std::to_string(frameRate.denominator()) + " is not supported by codec " + codecContext->codec->name);
	...
	{
		const enum AVPixelFormat *p = codecContext->codec->pix_fmts;
		if (*p == AV_PIX_FMT_NONE) throw FFmpegException("Codec " + std::string(codecContext->codec->name) + " does not have a default pixel format, you have to specify one");
		return *p;
	}
